...
impl ConformalKineticsSolver {
        ...
        F: Fn(&Float) -> Float,
    {
        ...
        let half =
            Float::with_val(self.precision, 0.5) * Float::with_val(self.precision, upper - lower);
        let center =
            Float::with_val(self.precision, 0.5) * Float::with_val(self.precision, upper + lower);
        ...
        for i in 0..8 {
            ...
            if i == 0 {
                gauss += positive * Float::with_val(self.precision, WG[3]);
            } else if i == 2 {
                gauss += Float::with_val(self.precision, &positive + &negative)
                    * Float::with_val(self.precision, WG[2]);
            } else if i == 4 {
                gauss += Float::with_val(self.precision, &positive + &negative)
                    * Float::with_val(self.precision, WG[1]);
            } else if i == 6 {
                gauss += Float::with_val(self.precision, &positive + &negative)
                    * Float::with_val(self.precision, WG[0]);
            }
        }
        let value = kronrod.clone() * &half;
        let error = ((kronrod - gauss) * &half).abs();
        if depth == 0 || error.to_f64() <= self.absolute_tolerance {
            return (value, error);
        }
        let midpoint = center;
