...
impl PyScreeningKernel {
    ...
    fn new(g_ef: f64, n_atom: f64, temp: f64, precision: Option<u32>) -> Self {
        ...
        Self {
            inner: crate::physics::screening::ScreeningKernel::new(
                Float::with_val(bits, g_ef),
                Float::with_val(bits, n_atom),
                Float::with_val(bits, temp),
            ),
        }
    }

    fn get_screening_potential(&self) -> f64 {
        self.inner.screening_potential.to_f64()
    }
...
impl PyConformalKineticsSolver {
    ...
    fn new(precision: u32) -> Self {
        Self {
            inner: crate::physics::cft_kinetics::ConformalKineticsSolver::new(precision),
        }
    }
    ...
    fn integrate_polynomial(
        ...
        py.allow_threads(|| {
            ...
            let value = solver.integrate_reaction_rate(
                |x| {
                    coefficients
                        .iter()
                        .rev()
                        .fold(Float::with_val(precision, 0), |acc, coefficient| {
...
impl PyMcNabbFosterSolver {
    ...
    fn new(
        ...
        let valid_traps = trap_density.len() == size
            ...
            && trap_density
                ...
                .zip(&capture_rate)
                .zip(&release_rate)
                .all(|((density, capture), release)| {
                    density.len() == capture.len() && density.len() == release.len()
                });
        if dx <= 0.0 || temperature <= 0.0 || !valid_profiles || !valid_traps {
            return Err(pyo3::exceptions::PyValueError::new_err(
                "invalid McNabb-Foster dimensions or parameters",
            ));
        }
        let profiles = diffusion_coeff
            .into_iter()
            .zip(solubility)
            .zip(partial_molar_volume)
            .map(|((diffusion_coeff, solubility), partial_molar_volume)| {
                crate::transport::mcnabb_foster::MaterialProfile {
                    diffusion_coeff,
                    solubility,
            ...
            .collect();
        let trap_populations: Vec<Vec<crate::transport::mcnabb_foster::TrapState>> = trap_density
            .into_iter()
            .zip(capture_rate)
            .zip(release_rate)
            .map(|((density, capture), release)| {
                density
                    .into_iter()
                    .zip(capture)
                    .zip(release)
                    .map(|((density, capture_rate), release_rate)| {
                        crate::transport::mcnabb_foster::TrapState {
                            density,
                            capture_rate,
                            release_rate,
                        }
            ...
            .collect();
        let theta = trap_populations
            .iter()
            .map(|traps| vec![0.0; traps.len()])
            .collect();
        Ok(Self {
            inner: crate::transport::mcnabb_foster::McNabbFosterSolver {
                grid: crate::transport::mcnabb_foster::TransportGrid {
                    dx,
                    size,
    ...
    fn concentrations(&self) -> Vec<f64> {
        self.inner.c_l.clone()
    }
}
...
impl PyRcwa3dSolver {
    ...
    fn build_system_matrix<'py>(
        &self,
        py: Python<'py>,
        permittivity: PyReadonlyArray2<'py, num_complex::Complex64>,
    ) -> Bound<'py, PyArray2<num_complex::Complex64>> {
