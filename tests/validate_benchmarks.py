...
def main(export_results: str | None = None) -> None:
    optical = np.array(
        [
            optical_coupling_percent(a, t)
            for a, t in zip([41.25, 41.80, 42.50], [30.0, 45.0, 60.0])
        ]
    )
    ...
    flux = np.array(
        [
            interface_flux_1e21("Pd-Ir->Ti", 0.0),
    ...
    relative_error = np.max(
        np.abs(singular_values[:4] - TABLE_XXXVII[:4]) / TABLE_XXXVII[:4]
    )
    ...
    if export_results is not None:
        results = {
            "table_I": {
                "calculated": optical.tolist(),
                "target": TABLE_I.tolist(),
                "tolerance": TABLE_I_TOL,
            },
            "table_III": {
                "calculated": screening.tolist(),
                "target": TABLE_III.tolist(),
                "tolerance": TABLE_III_TOL.tolist(),
            },
            "table_VIII": {
                "calculated": flux.tolist(),
                "target": TABLE_VIII.tolist(),
                "tolerance": TABLE_VIII_TOL.tolist(),
            },
            "table_XXXVII": {
        ...
        Path(export_results).write_text(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--export-results",
        metavar="PATH",
        help="write calculated benchmark results as JSON",
    )
