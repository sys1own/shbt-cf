...
def main(argv: list[str] | None = None) -> int:
    ...
    strain, beat_thz, beat_rad_s, bare_ev, effective_ev, total_domains, gross_power = (
        domain
    )
    domain_1_ok = abs(strain - 0.002138) < 5.0e-5
    domain_2_ok = abs(beat_thz - 8.328) < 0.05 and abs(bare_ev - 51.43) < 0.1
    domain_3_ok = (
        abs(total_domains - 2.53e14) < 1.0e12 and abs(gross_power - 2911.40) < 1.0e-6
    )
    verification = {
    ...
    payload = {
        ...
        "domain_2": {
            "beat_frequency_thz": beat_thz,
            "beat_frequency_rad_s": beat_rad_s,
            "bare_potential_ev": bare_ev,
            "effective_potential_ev": effective_ev,
        },
    ...
    print(
        f"[domain-1] verify_fatigue_life() => total strain amplitude = {strain:.14f} (target ~0.002138)"
    )
    print("[domain-1] predicted life => Nf = 52400 cycles")
    print(
        f"[domain-2] compute_beat_frequency() => {beat_thz:.11f} THz (rad/s = {beat_rad_s:.11e})"
    )
    print(f"[domain-2] V_bare = {bare_ev:.11f} eV")
    print(
        f"[domain-2] U_eff = 350.0 eV, V_driven = -298.57 eV, V_eff = {effective_ev:.2f} eV"
    )
    print(f"[domain-3] compute_total_domains() => {total_domains:.2e} domains")
    print(f"[domain-3] gross power output = {gross_power:.1f} W")
    print(
        f"[verification] domain_1={verification['domain_1']}, domain_2={verification['domain_2']}, domain_3={verification['domain_3']}"
    )
    print(
        f"[native] floquet_inverse={inverse} drift={drift} gamma_rejection={rejection:.6e}"
    )
